...
def get_args():
    ...
    parser.add_argument('--save_predictions', action='store_true', default=False,
                        help='Save per-image predictions to a json file in the output directory.')
...
def main(args, ds_init):
    ...
    if args.eval:
        ...
        # MODIFIED: Capture prediction_results and add saving logic
        test_stats, prediction_results = evaluate(data_loader_val, model, device)
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
        ...
        if args.save_predictions:
            if args.dist_eval:
                ...
                dist.all_gather_object(all_predictions_list, prediction_results)
                gathered_predictions = [item for sublist in all_predictions_list for item in sublist]
            else:
                gathered_predictions = prediction_results

            if utils.is_main_process():
                # Use a dict to remove duplicates from DistributedSampler padding
                unique_predictions = {p['image_path']: p for p in gathered_predictions}

                output_prediction_file = os.path.join(args.output_dir, "predictions.json")
                with open(output_prediction_file, "w") as f:
                    json.dump(list(unique_predictions.values()), f, indent=4)
                print(f"Predictions for {len(unique_predictions)} images saved to {output_prediction_file}")
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            # MODIFIED: Capture prediction_results
            test_stats, prediction_results = evaluate(data_loader_val, model, device)
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
            ...
            if max_accuracy < test_stats["acc1"]:
                ...
                if args.output_dir and args.save_ckpt:
                    ...
                    # --- ADDED: Save predictions for the best model ---
                    if args.save_predictions:
                        if args.dist_eval:
                            world_size = utils.get_world_size()
                            all_predictions_list = [None] * world_size
                        ...
                        if utils.is_main_process():
                            unique_predictions = {p['image_path']: p for p in gathered_predictions}
                            output_prediction_file = os.path.join(args.output_dir, "best_model_predictions.json")
                            with open(output_prediction_file, "w") as f:
                                json.dump(list(unique_predictions.values()), f, indent=4)
                            print(f"Best model predictions for {len(unique_predictions)} images saved to {output_prediction_file}")
