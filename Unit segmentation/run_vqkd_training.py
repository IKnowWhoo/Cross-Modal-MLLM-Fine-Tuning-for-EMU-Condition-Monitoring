...
def get_args():
    ...
    parser.add_argument('--vqkd_pretrained_path',
                        default='/data1/beit2/vqkd_encoder_base_decoder_3x768x12_clip-d5036aa7.pth', type=str,
                        help='path to pre-trained VQKD weights for initialization')
    parser.add_argument('--teacher_model_path', type=str,
                        default='/data1/beit2/packages/open_clip_pytorch_model.bin',
...
def main(args):
    ...
    # --- ADD THIS BLOCK TO LOAD PRE-TRAINED VQKD WEIGHTS ---
    if args.vqkd_pretrained_path:
        print(f"Loading pre-trained VQKD weights from {args.vqkd_pretrained_path}")
        checkpoint = torch.load(args.vqkd_pretrained_path, map_location='cpu')
        ...
        # Remove keys that might not match (e.g., optimizer states)
        # The 'as_tokenzer' path in modeling_vqkd.py already filters 'loss', 'teacher', 'scaling'
        keys_to_remove = ["loss", "teacher_model", "scaling_layer", "optimizer", "epoch"]
        filtered_state_dict = {k: v for k, v in state_dict.items() if not any(k.startswith(s) for s in keys_to_remove)}

        msg = model.load_state_dict(filtered_state_dict, strict=False)
        print(f"Loaded VQKD pre-trained weights. Missing keys: {msg.missing_keys}, Unexpected keys: {msg.unexpected_keys}")
    # --- END ADDED BLOCK ---
