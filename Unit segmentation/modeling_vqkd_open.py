...
class VQKD(nn.Module):
    def __init__(self,
        ...
        #     download_root="/data1/beit2/packages")
        #     self.decoder_out_dim = 512
        if self.teacher_model_type == 'clip':
            self.scaling_layer = ScalingLayerForClip()

            print("Loading LAION CLIP teacher model...")
            # 1. Create the model architecture.
            #    'ViT-B-16' is the correct name for the LAION model's architecture.
            self.teacher_model, _, _ = open_clip.create_model_and_transforms(
                'ViT-B-16',
                pretrained=teacher_model_path,  # Pass the path to your downloaded .bin file
                device='cpu'
            )

            self.teacher_model.eval()

            # 2. Set the correct feature dimension.
            #    The intermediate features from ViT-B/16 are 768-dimensional.
            self.decoder_out_dim = 512

        elif self.teacher_model_type == 'dino':
            self.scaling_layer = ScalingLayerForIM()
            # self.teacher_model = get_dino_vit_base()
            self.teacher_model = torch.hub.load('/data1/beit2/packages/dinov2_vitb14_reg4_pretrain.pth', 'dinov2_vitb14')
    ...
    def get_number_of_tokens_for_visualization(self):
    ...
    def get_regress_target(self, x, **kwargs):
        norm_imgs = self.scaling_layer(x)
        if self.teacher_model_type == 'clip':  # This logic now works for SigLIP
            # target = self.teacher_model.encode_image(norm_imgs, return_all_tokens=True) @ self.teacher_model.visual.proj
            orig_output_tokens = self.teacher_model.visual.output_tokens
            self.teacher_model.visual.output_tokens = True
            _, tokens = self.teacher_model.visual(norm_imgs)
            self.teacher_model.visual.output_tokens = orig_output_tokens
            target = tokens @ self.teacher_model.visual.proj
        #
        # elif self.teacher_model_type == 'dino':  # Original dino logic
        #     target = self.teacher_model.forward(norm_imgs, return_patch_tokens=True)
        elif self.teacher_model_type == 'dino':
            # 1. Get the DINOv2 patch tokens (B, 256, 768)
            target_features = self.teacher_model.forward_features(norm_imgs)['x_norm_patchtokens']

            # --- START NEW INTERPOLATION BLOCK ---

            # 2. Reshape to a 2D feature map for interpolation
            # B, L, C -> B, C, H, W
            B, L, C = target_features.shape
            H = W = int(L ** 0.5)  # Should be 16 for a 16x16 grid
            target_features_2d = target_features.permute(0, 2, 1).reshape(B, C, H, W)

            # 3. Interpolate from 16x16 to 14x14
            # The target size should match the student's output grid size
            student_grid_size = self.encoder.patch_embed.num_patches ** 0.5  # Should be 14
            target_features_resized = F.interpolate(
                target_features_2d,
                size=(int(student_grid_size), int(student_grid_size)),
                mode='bicubic',
                align_corners=False
            )

            # 4. Reshape back to a sequence of tokens
            # B, C, H, W -> B, L, C
            target = target_features_resized.reshape(B, C, -1).permute(0, 2, 1)

            # --- END NEW INTERPOLATION BLOCK ---
        else:
            raise NotImplementedError
    ...
    def calculate_rec_loss(self, rec, target):
        ...
        #     # Normalize the target features to have a consistent scale (unit variance).
        #     # This is crucial for stable training with L2 loss.
        #     target = F.layer_norm(target, target.shape[-1:])
        #     rec_loss = F.mse_loss(rec, target)
        if self.rec_loss_type == 'cosine':
            target = target / target.norm(dim=-1, keepdim=True).clamp(min=1e-6)
            rec = rec / rec.norm(dim=-1, keepdim=True).clamp(min=1e-6)
...
class ScalingLayerForIM(nn.Module):
    def __init__(self):
        ...
        self.register_buffer('shift', torch.Tensor([0.3043, 0.3043, 0.3043])[None, :, None, None]) # scale for tokenizer with default prosscess type \in [-1, 1]
        self.register_buffer('scale', torch.Tensor([0.2216, 0.2216, 0.2216])[None, :, None, None])
...
def vqkd_encoder_base_decoder_3x768x12_clip(pretrained=False, pretrained_weight=None, as_tokenzer=False, img_size=224, 
                                            n_code=8192, code_dim=32, teacher_model_path=None, **kwargs):
    encoder_config, decoder_config = get_model_default_params(), get_model_default_params()
    ...
    model = VQKD(encoder_config, decoder_config, n_code, code_dim, teacher_model_type=teacher_model_type, 
                 decoder_out_dim=decoder_out_dim, teacher_model_path=teacher_model_path, **kwargs)
